...
export async function runFillFormsUserTaskPageSF(opts?: F3Options) {
  ...
  for (let i=0; i<maxLoops; i++) {
    ...
    const fin = await postF3Static('detectFinalPage', { html: prevHtml }, log);
    ...
    // Analyze page once per HTML using STATIC mapping; reuse mapping if HTML unchanged
    let ana: any;
    if (lastAnaHtml === prevHtml && lastAna) {
      ana = lastAna;
      log(`SF-F3-Static analyze: reused cached static mapping for current HTML`);
    } else {
      ana = await postF3Static('analyzePageStaticFillForms', { html: prevHtml }, log);
    ...
    // CRITICAL FIELDS VALIDATION - key difference from LLM approach
    if (fieldMapping && Object.keys(fieldMapping).length) {
      const validation = await postF3Static('validateCriticalFields', { 
        field_mapping: fieldMapping, 
        ruhsat_json: ruhsat,
        task: "Yeni Trafik"  // TODO: make this dynamic based on user task
      }, log);
      
      if (validation?.ok) {
        log(`SF-F3-Static critical validation: success_rate=${validation.success_rate}, mapped=${validation.total_mapped}/${validation.total_critical}`);
        
        // Check if we should fallback to LLM
        const fallbackCheck = await postF3Static('checkShouldFallbackToLLM', { 
      ...
      const critical = ['plaka_no', 'model_yili', 'sasi_no', 'motor_no'];  // from config
      ...
      for (const k of order) {
        ...
        if (!okOne) { log(`SF-F3-Static step: give up ${k} after 3 attempts`); }
      }
      ...
      const filledCheck = await postF3Static('detectFormsFilled', { html: curHtml, min_filled: dynamicThreshold }, log);
