...
const CalibrationTopBar: React.FC<Props> = ({ host, task, darkMode, chipBorder, headerBorder, textMain, textSub, autoStatus, onTaskMenu, taskMenuOpen, loadTaskSuggestions, onClear, onToggleLogs, showLogs, onClose, docked, onRefresh }) => {
  return (
    <div style={{ padding:'12px 14px', borderBottom:`1px solid ${headerBorder}`, display:'flex', justifyContent:'space-between', alignItems:'center', gap:8 }}>
      ...
      <div style={{ display:'flex', gap:8, alignItems:'center', position:'relative' }}>
        ...
        {onRefresh && <button onClick={onRefresh} style={{ fontSize:12, padding:'6px 10px', border:`1px solid ${chipBorder}`, borderRadius:10, background: darkMode? '#1e3a8a':'#dbeafe', color:textMain }}>Refresh</button>}
        <button onClick={onToggleLogs} style={{ fontSize:12, padding:'6px 10px', border:`1px solid ${chipBorder}`, borderRadius:10, background: showLogs? (darkMode?'#052e2b':'#ccfbf1'): 'transparent', color:textMain }}>Loglar</button>
